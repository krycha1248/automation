...
resource "local_file" "inventory" {
  ...
  content  = <<-DYN
    ...
    ${ip} ansible_ssh_user=debian domain_name=${var.sub_domain}.wlodek.pro
    %{endfor~}
  DYN
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -T 600 -i dyn-inventory --private-key=~/ssh_key ../../ansible/meet.yml --vault-password-file=~/.vault"
  }
}